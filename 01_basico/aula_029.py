"""
CONSTANTE = "Variáveis" que não vão mudar
Muitas condições no mesmo if (ruim)
    <- Contagem de complexidade (ruim)
"""

velocidade = 61 # velocidade atual do carro
local_carro = 99 # local em que o carro está na estrada

RADAR_1 = 60 # velocidade máxima do radar 1
LOCAL_1 = 100 # local onde o radar 1 está
RADAR_RANGE = 1 # A distância onde o radar pega
RADAR_1_RANGE_MENOR = LOCAL_1 - RADAR_RANGE
RADAR_1_RANGE_MAIOR = LOCAL_1 + RADAR_RANGE

velocidade_carro_maior_radar_1 = velocidade > RADAR_1
carro_passou_radar_1 = local_carro >= RADAR_1_RANGE_MENOR and \
                        local_carro <= RADAR_1_RANGE_MAIOR
carro_multado_radar_1 = carro_passou_radar_1 and velocidade_carro_maior_radar_1

if velocidade_carro_maior_radar_1:
  print('Velocidade do carro acima do limite permitido')

if carro_passou_radar_1:
  print('Carro passou no radar 1')

if carro_multado_radar_1:
  print('Carro multado no radar 1')